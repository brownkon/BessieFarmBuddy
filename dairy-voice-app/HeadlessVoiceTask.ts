...
function parseSseResponse(rawText: string) {
  let fullContent = '';
...
export default async function HeadlessVoiceTask(taskData: any) {
  ...
  try {
    ...
    if (transcriptResult) {
      ...
      console.log('Response received from /api/chat stream.', { terminate: data.shouldTerminate });

      // Step D: Render assistant response in notification/overlay
      if (data.summary) {
          if (WakeWord.updateNotification) {
